...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    const defaults = getWorkerState().config.expect?.poll ?? {}, { interval = defaults.interval ?? 50, timeout = defaults.timeout ?? 1e3, message } = options, assertion = expect2(null, message).withContext({ poll: true });
